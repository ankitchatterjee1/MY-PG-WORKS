...
(COUNT(ibd.bid_status)/SUM(ibp.no_of_bids))*100 AS win_pct
from IPL_Bidding_Details ibd join  IPL_Bidder_Points ibp on ibd.bidder_id=ibp.bidder_id
where bid_status ='Won'  
group by ibd.bidder_id  order by win_pct DESC;


-- as per my assumption i got 27 row because according to qustion  the order of highest to lowest percentage.
...
select ims.stadium_id ,ips.stadium_name , ips.city  ,count(ims.match_id) from ipl_match_schedule ims join ipl_stadium ips
on ims.stadium_id=ips.stadium_id group by stadium_id ;

-- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?

-- solution --

select stadium_name ,city,(SUM(if(ipm.MATCH_WINNER=ipm.TOSS_WINNER,1,0))/COUNT(MATCH_ID))*100 as percentage_of_win_toss
from ipl_stadium as ips join ipl_match_schedule as ims using (STADIUM_ID)
join ipl_match as ipm using (MATCH_ID) 
group by stadium_name order by percentage_of_win_toss desc ;
...
join ipl_team ipt on ibd.bid_team = ipt.team_id using (bidder_id)
group by bid_team;
...
select match_id, if(ipl_match.match_winner =1,ipl_match.team_id1,ipl_match.team_id2) as win_team_id  from ipl_match; 

-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.

-- solution --

select team_id,team_name,sum(matches_played) as play_total_matches, sum(matches_won) as win_total_matches,
sum(matches_lost) as lost_total_matches from ipl_team_standings join ipl_team using  (team_id) group by team_id;

-- 7.	Display the bowlers for Mumbai Indians team.

-- solution --

select player_id,player_name,player_role,team_id,team_name from ipl_player join ipl_team_players
using (player_id) join ipl_team using (team_id)
...
select player_role,team_name,count(player_role) as no_of_allrounders from ipl_player ip join ipl_team_players itp
using (player_id) join ipl_team it using (team_id)
where player_role like '%all-rounder%'
group by  team_name having no_of_allrounders > 4 order by  no_of_allrounders DESC;
