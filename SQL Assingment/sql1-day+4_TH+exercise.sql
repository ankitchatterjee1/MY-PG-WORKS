...
select b.Account_Number, b.account_type, t.transaction_date, sum(t.Transaction_amount), h.Holiday
from bank_account_details b
left join bank_account_transaction t on b.Account_Number = t.Account_Number
left join bank_holidays h on t.Transaction_Date = h.Holiday
...
select t.account_number ,t.transaction_date ,h.holiday , m.event , m.customer_message 
from bank_account_transaction t 
inner join bank_holidays h on t.transaction_date=h.holiday 
cross join bank_customer_messages m
where m.event  = "adhoc" and month(t.transaction_date)=3 ;     
...
select b.account_number,b.account_type, t.transaction_date, t.transaction_amount, (t.transaction_amount*ra.interest_rate) as accrured_int
from bank_account_details b
inner join bank_account_transaction t on b.Account_Number = t.Account_Number
inner join bank_interest_rate ra on b.Account_type = ra.Account_type
where b.Account_type = 'recurring deposits' and datediff(curdate(),t.transaction_date) > 30;
...
select b.account_number as pri_ac_no, b.account_type pri_ac_type, r.account_number as sec_ac_no, r.account_type sec_ac_type, 
count(t.transaction_amount) as num_creditcard_trans
from bank_account_details b 
 join bank_account_relationship_details r on  b.Account_Number = r.Linking_Account_Number
 join bank_account_transaction t on r.Account_Number = t.Account_Number
where b.Account_type = 'savings' and r.Account_type like '%credit%'
...
select b.account_number as pri_no, b.account_type pri_type, r.account_number as sec_no, r.account_type sec_type,
count(t.transaction_amount) as num_creditcard_trans
from bank_account_details b 
 join bank_account_relationship_details r on  b.Account_Number = r.Linking_Account_Number
 join bank_account_transaction t on r.Account_Number = t.Account_Number
where b.Account_type = 'savings' and r.Account_type = 'Add-on Credit card'
group by b.Account_Number
having num_creditcard_trans >= 1 ;

-- there are no transactions that are made more than once in 'Add-onCreditcard'.
...
select customer_id, length(customer_id)                                                                               -- CHECK
from bank_customer
...
select * from bank_customer left join bank_customer_export using (customer_id) ; 
